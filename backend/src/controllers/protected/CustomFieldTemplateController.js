...
const createEditCustomField = async (req, res) => {
  try {
    ...
    if (
      existingCustomField &&
      (!id || existingCustomField._id.toString() !== id)
    ) {
      throw new CustomError(
        400,
        "We already have custom fields for this post type!"
      );
    }
    ...
    updatedCustomField = {
      ...updatedCustomField.toObject(),
      id: updatedCustomField._id,
    };
...
const deleteCustomField = async (req, res) => {
  try {
    ...
    ResponseHandler.success(
      ...
      {
        message:
          "CustomField and associated PostMeta (if present) deleted successfully.",
      },
      200
    );
  } catch (error) {
