        ...
        <Form {...form}>
            <div className="">
                ...
                >
                    ...
                    <>
                        <h1 className="text-main-bg-900 text-2xl border border-t-0 border-x-0 border-b-main-bg-900">Custom Fields</h1>
                        <div className="flex flex-col w-3/4 gap-8">
                            <DynamicForm fields={customFormFields} postFormData={postFormData} setPostFormData={setPostFormData} />
                        </div>
                    </>

                    <h1 className="text-main-bg-900 text-2xl border border-t-0 border-x-0 border-b-main-bg-900">Seo Data</h1>
                                        ...
                                        <FormControl>
                                            <Select onValueChange={field.onChange} defaultValue={field.value || 'select'}>
                                                <SelectTrigger className="w-full">
                                                    <SelectValue>{/*@ts-ignore*/}
                                                        {(!field.value || field.value === 'select' || !seoFilePath.includes(field?.value))
                                                            ? 'Default Schema'
                                                            : formatOption(field.value)}
                                                    </SelectValue>
                                                </SelectTrigger>
                                                <SelectContent>
                                                    <SelectItem key={'select'} value={'select'}>Default Schema</SelectItem>
                                                    {!isFilePathLoading && seoFilePath.map((option) => {
                                                        return (
                    ...
                    {seoContent && (
                        <><h1 className="text-main-bg-900 text-2xl border border-t-0 border-x-0 border-b-main-bg-900">Seo Content</h1>
                            <pre className="text-balance">{JSON.stringify(seoContent, null, 2)}</pre></>
                    )}
                    {canEdit && <Button type="submit" className="shad-button_primary max-w-fit self-end min-w-24" disabled={isOperating} >
